 ...
 * File: lfo_random.cpp
 *
 * Simple runtime test using LFO class as audio rate oscillator. Modified to choose modulation
 *  oscillator at random per call to process an audio frame.
 *
 * 
 * 
 * 2018 (c) Korg
                   ...
                   uint32_t frames)
{
  ...
  for (; my != my_e; ) {
    ...
    // Pick a modulation oscillator wave type at random
    s_lfo_wave = si_roundf(si_fabsf(osc_white()) * (k_wave_count - 1));
